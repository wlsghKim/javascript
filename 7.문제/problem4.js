...
// 3. 남자회원 나이의 총합과 여자회원 나이의 총합?
{
  ...
  console.log(`여자회원의 나이 총합=${ageOfWemen}`);
  console.log(`남자회원의 나이 총합=${ageOfMen}`);
  }
}
...
{   //case3) map, reduce, forEach, set
  const hobby3 = result.members.map(m => m.hobby)
                .reduce((acc,hobby) => acc.concat(hobby) ,[]);  
  (new Set(hobby3)).forEach(hobby => console.log(hobby));
}
...
// 7. 모든 회원이 여성회원들로만 이루어져있는지 판단하시오.
{
