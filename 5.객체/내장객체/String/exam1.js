...
{
    const str = '울산KH정보교육원';
    console.log(str.endsWith('원'));
    console.log(str.endsWith('윈'));
}
{
    //includes
    const str = '울산KH정보교육원';
...
{
    //test : 주어진 문자열에서 정규표현식과 일치하면 true 블일치 false.
    const str = '010-1234-5678';
    const re = /^010-\d{4}-\d{4}$/;
    // const re = new RegExp('010-\\d{4}-\\d{4}');
...
{
    //RegEx.property.test : 주어진 문자열에서 정규표현식과 일치하면 true 블일치 false.
    const str = '010-1234-5678';
    const re = /^010-\d{4}-\d{4}$/;
    // const re = new RegExp('010-\\d{4}-\\d{4}');
    console.log(re.test(str));
...
}
{
    ...
    console.log(str.substring(-2)); //음수불가
